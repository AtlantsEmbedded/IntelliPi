function [ldadata, eigvector, eigvalue] = fda(gnd, options, data)
...
%       
%       fea = rand(60,4096);
%       gnd = [ones(20,1);ones(20,1)*2;ones(20,1)*3];
%       options = [];
%       options.Fisherface = 1;
%       [eigvector, eigvalue] = FDA(gnd, options, fea);
%       Y = fea*eigvector;
%       figure; gscatter(Y(:,1), Y(:,2), gnd, ['g' 'b' 'r' 'y'])
% 
%
...
% Determine the unique classes label, the number of them and dim = n-1
classLabel = unique(gnd);
nClass = length(classLabel);
Dim = nClass - 1;

% Compute the ration, nb of sample - nb of classes
if bPCA & isfield(options,'Fisherface') & options.Fisherface
    options.PCARatio = nSmp - nClass;
end

% If the data matrix is sparse, reduce it to full representation
if issparse(data)
    data = full(data);
end

% Remove the mean from the data
% By using a replicated matrix of nSmp size, filled with the mean value.
...
if bPCA    
    if nSmp > nFea
        ...
        % Eliminates eigenvalues that are very small
        eigIdx = find(eigvalue_PCA/maxEigValue < 1e-12);
        eigvalue_PCA(eigIdx) = [];
        eigvector_PCA(:,eigIdx) = [];

        % Sort the data from in ascending order (inverse)
        ...
        if options.PCARatio > 1
            % Keep only the a number of eigenvalues equal to the ratio
...
%% the output of the previous step is data
