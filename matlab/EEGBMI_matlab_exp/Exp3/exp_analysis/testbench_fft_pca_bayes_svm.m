...
%                                                                                       *
%                 Pre-processing:                                                       *
%                 Apply the fast-fourier transform to the signal.                       *
%                                                                                       *
%                 A PCA is applied to reduce the dimensionality and select a subset     *
%                 of the dimensions.                                                    *
%                                                                                       *
...
%                                                                                       *
%****************************************************************************************
function testbench_fft_pca_bayes_svm(dataset, normalize, window_length, trainset_proportion)
...
% fft the timeseries
[ feature_vectors_original ] = fft_data( trials_data );
...
for ii=1:bootstrap_iter
    ...
    [eigvector, pcadata, eigvalue] = pca(train_vectors);
    
    pcadata = pcadata(:,1:15);
    
    % transform test data
    pcatestdata = test_vectors*eigvector(:,1:15);
    
    % train bayesian decoder
    O1 = NaiveBayes.fit(pcadata,train_labels);
    % predict the test labels
    C1 = O1.predict(pcatestdata);
    ...
    cl = svmtrain(pcadata,train_labels,'kernel_function','rbf','BoxConstraint',Inf);
    svm_predicts = svmclassify(cl,pcatestdata);
