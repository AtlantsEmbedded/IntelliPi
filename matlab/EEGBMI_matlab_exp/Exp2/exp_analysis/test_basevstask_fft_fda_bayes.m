...
[ feature_vectors_original, baseline_vectors_original ] = fft_data( trials_data, baseline_data );

labels_original = [zeros(size(feature_vectors_original,1),1);zeros(size(baseline_vectors_original,1),1)+1];
feature_vectors_original = [feature_vectors_original;baseline_vectors_original];

% define the number of data that will be used for training
nb_train = 260;
nb_test = size(feature_vectors_original,1)-nb_train;
...
for ii=1:bootstrap_iter
    ...
    % permute the samples
    perm_idx = randperm(size(feature_vectors_original,1));

    % make copy of permutated data
    feature_vectors = feature_vectors_original(perm_idx,:);
    labels = labels_original(perm_idx);
