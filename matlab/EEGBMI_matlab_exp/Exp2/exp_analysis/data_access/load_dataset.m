function [ trials_data, baseline_data, record_trials_info ] = load_dataset( dataset_id )
...
baseline_data = cell(nb_record_trials,1);
...
for ii=1:nb_record_trials
    ...
    load(['dataset_' num2str(dataset_id) '/trial_' num2str(ii) '_base']);
    
    if size(IXDATA.raw.eeg.data,1)<window_length
        fprintf('base_trial: %i dropped\n',ii);
        dropped_base_trials = [dropped_base_trials ii];
    else
        temp_data = IXDATA.raw.eeg.data((end-(window_length-1)):end,:);
        baseline_data{ii} = temp_data;
    end
    
end


% for missing baseline, take the average of the two valids that are the "closest"
for ii=dropped_base_trials
    ...
    if ii<10
        ...
        while kk < 2
            ...
            if ~isempty(baseline_data{jj})
                sum_of_closest = sum_of_closest+baseline_data{jj};
                kk=kk+1;
            end
        ...
        baseline_data{ii} = sum_of_closest./2;
        
    elseif ii>numel(baseline_data)-10
        
        sum_of_closest = zeros(window_length,4);
        
        jj=numel(baseline_data)-1;
        kk=0;
