...
int main(int argc, char **argv[])
{
	...
	register float MeanSquareError;
	...
	for (kk = 0; kk < NbOfBatch; kk++) {
		...
		for (ii = 0; ii < TrainingLenght; ii++) {
			...
			/*c ompute and log the desired output, network output and mean square error */
			MeanSquareError = 0;
			for (jj = 0; jj < 8; jj++) {
				MeanSquareError +=
 ...
 * @note Be careful with this function as it is NON-REENTRANT!
 */
static inline struct timespec diff(struct timespec start, struct timespec end)
{
