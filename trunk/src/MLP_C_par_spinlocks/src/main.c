 /**
 * @file main.c
 * @author Frederic Simard
 * @date October, 2014
 *
 * @brief This file implements the program that runs a MLP algorithm on a set of data.
 * What it does:
 * - load the dataset
 ...
 * @brief Like an opinion, everyone has one
 * @param argc
 * @param argv[]
...
{
	 register int kk = 0;	/*batch counter */
	register int ii = 0;	/*training counter */
	int NbOfBatch = 25;	/*nb of batchs */
	register int TrainingLenght = 1000;	/*nb of trials per batch */
	 register int jj = 0;	/*General Purpose counter */
	...
	strcpy(TrainDataSetInitProp.FileName, TRAINDATASET_FILE);
	...
	strcpy(TestDataSetInitProp.FileName, TESTDATASET_FILE);
	...
	struct SNNetworkProp NetworkInitProp;
	    ...
	    /*init the log file */ 
	    FILE * ScoreLog = NULL;
	ScoreLog = fopen(SCOREFILE_NAME, "w");
	    ...
	    for (kk = 0; kk < NbOfBatch; kk++)
		    ...
		    for (ii = 0; ii < TrainingLenght; ii++)
			    ...
			    GetRandomTrial(pTrainDataSet, &Input,
					   &TrialSolution);
			    ...
			    MeanSquareError = 0;
				    ...
				    (TrialSolution[jj] -
				     NetworkOutput[jj]) * (TrialSolution[jj] -
							   NetworkOutput[jj]);
	    ...
	    printf("Exec. Time = %d:%d\n", (int)tot.tv_sec, (int)tot.tv_nsec);
 ...
 * 
 * @brief timer operation used for profiling
