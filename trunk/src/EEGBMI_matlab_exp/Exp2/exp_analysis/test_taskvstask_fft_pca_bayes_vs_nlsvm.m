...
% center the data
avg_vector = mean(feature_vectors_original);

for ii=1:size(feature_vectors_original,2)
    feature_vectors_original(:,ii) = feature_vectors_original(:,ii) - avg_vector(ii);
end
...
for ii=1:bootstrap_iter
    ...
    [COEFF,SCORE,latent] = princomp(train_vectors);
    
    % transform test data
    testdata = train_vectors*COEFF(:,1:2);
    test_labels = train_labels;
    
    % train bayesian decoder
    O1 = NaiveBayes.fit(SCORE(:,1:2),train_labels);
    ...
    %cl = svmtrain(SCORE(:,1:3),train_labels,'kernel_function','rbf','BoxConstraint',Inf);
    cl = svmtrain(SCORE(:,1:2),train_labels);
